...
- **플레이어와 엮기.** `possessable_component`는 이름만 `<public>`이고 클래스가
  `<epic_internal>`이라 Verse에서 생성할 수 없고, `possessor_component`는 통째로
  `<internal>`이라 빙의를 일으킬 수단이 없습니다. 게다가 플레이어 폰은 씬 그래프에
  존재하지 않습니다 (문서의 벽 ⑫).
