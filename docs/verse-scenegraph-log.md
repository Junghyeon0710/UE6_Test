...
### 벽 ⑯ — Verse에 공개 입력 API가 없다

엔진의 Verse 파일 전체를 뒤졌지만 입력을 받는 공개 경로가 없습니다.

```verse
input_action<native><public>(t:type) := class<epic_internal><allocates><unique><computes>(asset):
                              ^^^^^^^^                     ^^^^^^^^^^^^^
```

또 벽 ⑫와 같은 패턴입니다 — 이름만 `<public>`, 클래스는 `epic_internal`.
게다가 `input_action`을 **소비하는 함수가 어디에도 없습니다.**
`Pressed` / `OnInput` / `KeyDown` 류의 공개 심볼도 0개입니다.

### 그래서 입력의 "결과"를 읽는다

플레이어 폰은 13장의 C++ 브릿지로 `LevelEntity` 트리 안에 들어와 있습니다.
그러니 **플레이어 엔티티의 트랜스폼 변화**를 읽으면 입력의 결과를 알 수 있습니다.

[`InputRelayComponent.verse`](../Plugins/VerseGame/Content/InputRelayComponent.verse)의
`input_reaction_component`는 트리에서 플레이어를 찾아 위치를 샘플링하고,
수평 이동 속도를 계산해 자기 라이트에 반영합니다.

구동은 `add_movement_input` / `jump` 로 했습니다. **Move / Jump 입력 액션이 부르는
바로 그 함수**라, 키를 누르는 것과 같은 경로를 탑니다.

### scene_event 방식은 실패했다

처음엔 밀어내는(push) 구조로 만들었습니다. 플레이어에 붙인 relay가
`scene_event`를 만들어 트리에 `SendDown` 하고, 비콘들이 `OnReceive`로 받는 방식입니다.

`scene_event`는 `public interface`라 구현할 수 있고, `OnReceive` 오버라이드와
`SendDown` 호출 모두 **컴파일은 통과했습니다.** 그런데 실제로는 이벤트가
도착하지 않았습니다(검증용 스케일이 1.0에서 미동도 없음).

`SendDown` 문서는 하위 전체로 전파된다고 되어 있으니, 플레이어 엔티티에서
`GetParent`를 타고 올라간 루트가 비콘들의 조상까지 닿지 않은 것으로 보입니다.
원인을 끝까지 추적하지는 않았고, **검증된 순회(pull) 방식으로 바꿔서 해결**했습니다.
14장의 근접 컴포넌트가 쓰던 것과 같은 순회입니다.

### 벽 ⑰ — Verse 클래스를 지우면 그걸 참조하던 에셋이 조용히 망가진다

relay를 파일에서 삭제했더니, 그 클래스를 참조하던 `Prefab_Player`에서
**relay뿐 아니라 `sphere_light_component`까지 함께 사라졌습니다.**
경고 한 줄 없었고, PIE에서 플레이어 엔티티의 컴포넌트를 찍어보고서야 알았습니다.

### 검증

Verse 프로퍼티는 파이썬에서 못 읽으므로(9장), 계산된 비율을 엔티티 **스케일**에
기록하게 했습니다. 메시가 없어 화면에는 안 보이지만 `GetEntityTransform`으로
읽을 수 있습니다.

```
     상태     비콘 3개의 스케일
     정지     [1.000, 1.000, 1.000]
   이동 중     [1.089, 1.089, 1.089]
   이동 중     [1.124, 1.124, 1.124]
   이동 중     [1.057, 1.057, 1.057]
   이동 중     [1.165, 1.165, 1.165]
  정지 후     [1.000, 1.000, 1.000]
```

셋이 항상 같은 값으로 움직이고, 멈추면 정확히 1.0으로 돌아옵니다.

> 처음엔 이동이 끝난 **뒤에** 스케일을 읽어서 계속 1.0이 나왔습니다.
> 살아있는 값이라 움직이는 **동안** 샘플링해야 합니다.

### 점프 판정 — 절대 높이에서 상대 높이로

처음엔 `Now.Up > 330.0` 처럼 **절대 Z**로 판정했는데, 지형 높낮이가 다르면
그대로 틀립니다. 실제로 테스트 중 캐릭터가 Z=248에 있어 임계값 아래라
점프가 인식되지 않았습니다.

지면 기준을 **추적하는** 방식으로 바꿨습니다. 수직 속도가 거의 없으면
"지면에 안착했다"고 보고 그때의 높이를 기준으로 갱신합니다.

```verse
set VertSpeed = (Now.Up - Prev.Up) / UpdateInterval

# 수직 이동이 거의 없으면 지면에 안착한 것으로 보고 기준을 갱신
...
`IsAlmostZero`는 `/Verse.org/Verse`의 공개 확장 함수입니다(`<decides>`라 대괄호 호출).
경사를 오르내려도 기준이 따라오므로 지형에 독립적입니다.

검증 — 높이가 약 200 차이나는 두 지대에서 각각 점프시켰습니다.
(스케일의 `Forward`에 속도비율, `Up`에 공중 여부를 실어 읽었습니다)

```
[낮은 지대] 착지 Z= 96.5   지상판정 Up=1.0
            점프 최고 Z=188.2   공중판정 Up=2.0
            착지 후 Up=1.0

[높은 지대] 착지 Z=292.4   지상판정 Up=1.0
            점프 최고 Z=356.6   공중판정 Up=2.0
            착지 후 Up=1.0
```

낮은 지대의 점프 최고점(188)은 예전 임계값(330)에 한참 못 미쳐 **절대 방식으로는
절대 잡히지 않던** 경우입니다. 이제 두 지대 모두 정확합니다.

### 낙하 구분 — 수직 속도의 부호

기준 높이 방식만으로는 **낙하를 못 잡습니다.** 높은 곳에서 떨어지는 동안에는
수직 속도가 커서 기준이 갱신되지 않고, 그 기준이 출발 지점(높은 곳)에 머물러
있어 `Now.Up - GroundLevel`이 음수가 되기 때문입니다.

상승과 낙하를 각각 다른 근거로 판정하도록 나눴습니다.

```verse
# 기준보다 위로 떠 있으면 상승(점프)
Rising  := if (HasGround?, Now.Up - GroundLevel > AirborneHeight) then true else false
# 아래로 빠르게 움직이면 낙하 - 기준 높이로는 안 잡히므로 속도 부호로 본다
Falling := if (VertSpeed < -FallSpeed) then true else false
Airborne := if (Rising?) then true else Falling
```

`FallSpeed`(기본 120 cm/s)는 `SettleSpeed`(20)보다 넉넉히 크게 잡아, 경사를
걸어 내려갈 때 낙하로 오검출되지 않게 했습니다.

검증 — 스케일 `Up`에 세 상태를 실었습니다 (1.0 지면 / 2.0 상승 / 3.0 낙하).

```
[점프] 지면에서 뛰어오르기
  지면(Z=122) -> 상승(Z=183) -> 낙하(Z=102) -> 지면(Z=97)

[낙하] 공중에 놓고 떨어뜨리기 (점프 없음)
  ... -> 낙하(Z=97) -> 지면(Z=97)
```

점프 한 번에 **상승 → 낙하 → 착지**가 순서대로 잡힙니다.

> 참고: 캐릭터를 순간이동시키면 한 샘플 동안 수직 속도가 폭증해 "상승"으로
> 잡힙니다. 낙하 테스트를 위해 Z=1200으로 텔레포트했을 때 실제로 그랬습니다.
> 정상 플레이(난간에서 걸어 나가기 등)에서는 발생하지 않습니다.

### 남은 한계

- 스크립트로 `add_movement_input`을 호출하는 방식은 프레임 정렬이 맞지 않아
  실제 키 입력만큼 지속적인 속도가 안 나옵니다. 측정된 비율(0.05~0.17)이
  낮은 이유입니다. 사람이 직접 키를 누르면 더 큰 값이 나옵니다.
...
| ⑯ | Verse에 공개 입력 API가 없다 | 엔진 Verse 파일 전수 검색 |
| ⑰ | Verse 클래스를 지우면 참조하던 에셋의 컴포넌트가 조용히 사라짐 | PIE에서 컴포넌트 목록 확인 |
...
15. 살아있는 값은 그 일이 **일어나는 동안** 읽어야 한다
16. 컴파일이 통과해도 런타임에 동작한다는 보장은 없다 (scene_event 전파)
